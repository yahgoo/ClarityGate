...
def write_report(
    ...
    out_path: str | Path = "claritygate-report.md",
) -> ReportResult:
...
def render_report(
    ...
    lines = [
        ...
        f"- Quest Readiness Score: **{evaluation.score}/100** ({evaluation.tier})",
        "",
        "## Summary",
        "",
    ...
    if evaluation.findings:
        ...
        lines.append("No findings. The spec is ready for Qoder handoff.")
    ...
    if clarifications:
        for index, finding in enumerate(clarifications, start=1):
            lines.append(
                ...
                f"A) Keep as-is and accept AI drift risk; B) {finding.suggested_rewrite}"
            )
    else:
...
def print_summary(report: ReportResult, evaluation: EvaluationResult) -> None:
    ...
    print("ClarityGate scan complete")
    ...
    print(f"Quest Readiness Score: {evaluation.score}/100 ({evaluation.tier})")
