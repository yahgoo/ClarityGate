"""ClarityGate CLI orchestration for the five-stage linter pipeline."""
...
def run(input_path: str, out_path: str = "claritygate-report.md") -> int:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Scan requirements.md for ClarityGate quality issues.")
    parser.add_argument("path", help="Path to a requirements.md file")
    parser.add_argument("--out", default="claritygate-report.md", help="Markdown report output path")
    args = parser.parse_args(argv)

    try:
        return run(args.path, args.out)
    except LoadError as exc:
        print(f"ClarityGate load error: {exc}", file=sys.stderr)
        return 2
